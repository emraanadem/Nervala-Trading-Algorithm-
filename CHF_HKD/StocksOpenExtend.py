...
class History:
    ...
    def Equalizer(instrument):
        ...
        data = supabase.table('Five_Min Extend') \
            .insert({"id": ids, "Instrument": instrument, "Data": History.five, "OHLC": "o"}) \
            .execute()
        data = supabase.table('Fifteen_Min Extend') \
            .insert({"id": ids, "Instrument": instrument, "Data": History.fifteen, "OHLC": "o"}) \
            .execute()
        data = supabase.table('Thirty_Min Extend') \
            .insert({"id": ids, "Instrument": instrument, "Data": History.thirty, "OHLC": "o"}) \
            .execute()
        data = supabase.table('One_Hour Extend') \
            .insert({"id": ids, "Instrument": instrument, "Data": History.hour, "OHLC": "o"}) \
            .execute()
        data = supabase.table('Two_Hour Extend') \
            .insert({"id": ids, "Instrument": instrument, "Data": History.twohour, "OHLC": "o"}) \
            .execute()
        data = supabase.table('Four_Hour Extend') \
            .insert({"id": ids, "Instrument": instrument, "Data": History.fourhour, "OHLC": "o"}) \
            .execute()
        data = supabase.table('Daily Extend') \
            .insert({"id": ids, "Instrument": instrument, "Data": History.daily, "OHLC": "o"}) \
            .execute()
        data = supabase.table('Weekly Extend') \
            .insert({"id": ids, "Instrument": instrument, "Data": History.weekly, "OHLC": "o"}) \
            .execute()
