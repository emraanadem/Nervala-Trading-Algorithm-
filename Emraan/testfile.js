...
class One_Hour_Nexus{
    ...
    static piplogger(){
        ...
        if (this.buy_pos){
            piplogging.push(One_Hour_Functions.pipCountBuy(this.posprice, One_Hour_Functions.price))
            this.bigpipprice = Math.max(...piplogging)
            this.piplog = piplogging}
        if (this.sell_pos){
            piplogging.push(One_Hour_Functions.pipCountSell(this.posprice, One_Hour_Functions.price))
            this.bigpipprice = Math.max(...piplogging)
            this.piplog = piplogging}
    }
    /**take profit for buying */
    static takeProfitBuy(){
        if (One_Hour_Functions.price >= this.tp){
            if(One_Hour_Functions.volatility() > .618){
                if((One_Hour_Functions.price - this.tp) > (this.tp - this.tstoploss)){
                        if(this.tp < this.tptwo){
                            One_Hour_Nexus.piploginit()
                            this.posprice = this.tp
                            this.tp = this.tptwo
                            ...
                            console.log("New Target Take Profit: " + String(this.tp))
                            console.log("New Take Profit 2: " + String(this.tptwo))
                            }}}
            else{
            One_Hour_Nexus.closePosTP()}}
        else if (One_Hour_Functions.price <= this.tstoploss){
            One_Hour_Nexus.closePosTP()}
        else if (One_Hour_Functions.price == this.tptwo){
            One_Hour_Nexus.closePosTP()}
    }
    ...
    static takeProfitSell(){
        if (One_Hour_Functions.price <= this.tp){
            if(One_Hour_Functions.volatility() > .618){
                if((this.tp - One_Hour_Functions.price) > (this.tstoploss - this.tp)){
                        if(this.tp < this.tptwo){
                            One_Hour_Nexus.piploginit()
                            this.posprice = this.tp
                            this.tp = this.tptwo
    ...
    static stoplossdef(){
        ...
        if(this.buy_pos){
            this.sl = One_Hour_Functions.price - stoploss}
        if(this.sell_pos){
            this.sl = One_Hour_Functions.price + stoploss}}
    ...
    static tstoplossinit(){
        let stoploss = this.sldiff
        if(!One_Hour_Nexus.tstop && !One_Hour_Nexus.tstoplossinits && !One_Hour_Nexus.tstoplossvoid){
            if(this.buy_pos){
                if(One_Hour_Functions.price > this.posprice + .3*stoploss){
                    this.tstoplossinits = true
    ...
    static tstoplossdefvol(){
        this.sldiff = One_Hour_Functions.stoploss()
        let stoploss = this.sldiff
        if(this.buy_pos){
            if(One_Hour_Functions.price > this.posprice + .3*stoploss){
                this.tstop = true
                this.tstoploss = this.posprice + (((Math.abs(One_Hour_Functions.price-this.posprice))*(One_Hour_Functions.trailingsl())))
            }}
        if(this.sell_pos){
            if(One_Hour_Functions.price < this.posprice - .3*stoploss){
                this.tstop = true
    ...
    static tstoplosscheck(){
        let tstoploss = this.sldiff
        if(this.buy_pos){
            if(One_Hour_Functions.price < this.posprice + .3*tstoploss){
                this.tstoplossvoid = true
            }
            else{
                this.tstoplossvoid = false
    ...
    static tstoplosscont(){
        if(One_Hour_Functions.volatility() < .618 && this.tstoplossinits && !this.tstoplossvoid){
            this.sldiff = One_Hour_Functions.stoploss()
            let stoploss = this.sldiff
            if(this.buy_pos){
                if(One_Hour_Functions.price > this.posprice + .3*stoploss){
                    this.tstoploss = this.posprice + One_Hour_Functions.pipreverse(this.posprice, .618*this.bigpipprice)
            }}
            if(this.sell_pos){
                if(One_Hour_Functions.price < this.posprice - .3*stoploss){
                    this.tstoploss = this.posprice - One_Hour_Functions.pipreverse(this.posprice, .618*this.bigpipprice)
    ...
    static buy(){
        ...
        if(!One_Hour_Functions.rejectionzoning()){
            if (Math.abs(One_Hour_Functions.valdiff(One_Hour_Functions.price, One_Hour_Functions.closest(One_Hour_Functions.price))) > .1) {
                this.tp = this.resistance
                this.pos = true
                this.buy_pos = true
                this.posprice = One_Hour_Functions.price
                One_Hour_Nexus.stoploss()
                One_Hour_Nexus.tpvariation()
                console.log('pair: ' + this.pair)
                console.log("Open Buy Order on One Hour")
                console.log("Entry Price: " + String(this.posprice))
                console.log("Stop Loss: " + String(this.sl))
                console.log("Target Take Profit: " + String(this.tp))
                console.log("Take Profit 2: " + String(this.tptwo))
    ...
    /*static buy(){
        ...
        console.log(this.sl + " : Stop Loss")
        console.log(this.tp + " : Target Take Profit")
        } */
    ...
    static controlBiggerPeriod(){
        ...
        this.biggersupres = totallevs
        this.finlevs.concat(totallevs)
    }
    ...
    static closePosTP(){
        if (this.pos){
            if (this.buy_pos){
                ...
                this.trades += 1
                this.piplog = [0, 0, 0]
                let pipchange = One_Hour_Functions.pipCountBuy(this.posprice, One_Hour_Functions.price)
                this.pips += Math.abs(pipchange)
                console.log('pair: ' + this.pair)
            ...
            if (this.sell_pos){
                ...
                this.trades += 1
                this.piplog = [0, 0, 0]
                let pipchange = One_Hour_Functions.pipCountSell(this.posprice, One_Hour_Functions.price)
                this.pips += Math.abs(pipchange)
                console.log('pair: ' + this.pair)
                console.log("Take Profit Hit on One Hour")
                console.log(One_Hour_Nexus.wins + " Wins and     " + One_Hour_Nexus.losses + " Losses")
...
/** load historical prices from json file */
    static HistoryAssigner(){
        ...
        try{
            ...
            this.priceHist = dataspecific
        }catch (error) {}
        try{
            let data = JSON.parse(rawtwo)
            let dataspecific = data[instrument]['One_Hour']
            this.highs = dataspecific
        }catch (error) {}
        try{
            let data = JSON.parse(rawthree)
            let dataspecific = data[instrument]['One_Hour']
            this.lows = dataspecific
        }catch (error) {}
        try{
            let data = JSON.parse(rawfour)
            let dataspecific = data[instrument]['One_Hour']
            this.extendHist = dataspecific
        }catch (error) {}
        try{
            let data = JSON.parse(rawfive)
            let dataspecific = data[instrument]['One_Hour']
            this.extendHigh = dataspecific
        }catch (error) {}
        try{
            let data = JSON.parse(rawsix)
            let dataspecific = data[instrument]['One_Hour']
            this.extendLow = dataspecific
        }catch (error) {}
        }
...
/** second consolidation method, meant to strengthen consolidation identification */
    ...
    static tpvariation(){
        ...
        let values = One_Hour_Nexus.finlevs.push(One_Hour_Nexus.biggersupres)
        let valdiffgreater = []
        let valdiffless = []
        let closesttp = 0
        let filteredvaldiff = []
    ...
    */
    ...
    static rejecinit(){
        ...
        if(!fs.existsSync('./Rejection_Archive/'+String(instrument)+'.json')){
            this.timeperiods = {}
            this.timeperiods['Fifteen_Min'] = [0, 0, 0]
            this.timeperiods['Thirty_Min'] = [0, 0, 0]
            this.timeperiods['One_Hour'] = [0, 0, 0]
            this.timeperiods['Two_Hour'] = [0, 0, 0]
            this.timeperiods['Four_Hour'] = [0, 0, 0]
            this.timeperiods['Daily'] = [0, 0, 0]
            this.timeperiods['Weekly'] = [0, 0, 0]
            fs.writeFileSync('./Rejection_Archive/'+String(instrument)+'.json', JSON.stringify(this.timeperiods, null, 2))
        }
        let raw = fs.readFileSync('./Rejection_Archive/'+String(instrument)+'.json')
        this.timeperiods = JSON.parse(raw)
        this.rejectionzones = JSON.parse(raw)['One_Hour']
    }
    /** Rejection Zone Saver */
    static rejecsave(){
        let instrument = One_Hour_Functions.instrument_name()
        this.rejectionzones = [...new Set(this.rejectionzones)]
        this.timeperiods['One_Hour'] = this.rejectionzones
        fs.writeFileSync('./Rejection_Archive/'+String(instrument)+'.json', JSON.stringify(this.timeperiods, null, 2))
    }
...
/**  Machine learning method used to determine past movement patterns at different prices, can help with stop loss and take profit definition*/
    static overall(){
        let extendedhistory = this.extendHist
        ...
        for(let val = 0; val < fractals.length; val++){
            ...
            if(mincount || maxcount > 4){
                ...
                if(fractals.length < 1){
                    fractals.push(0)
                    this.rejectionzones.push(fractals[0])
                }else{
                    let frac = fractals[val]
                    this.rejectionzones.push(extendedhistory[frac])
            }}}
        if(this.rejectionzones.length < 1){
            this.rejectionzones.push(One_Hour_Functions.price)
        }
        if(rejection > 2){
            return false
        }else{
...
/** Smart array that grows as program runs longer for each time period, shows rejection zones and if the program is near them, it'll not allow trading */
    static rejectionzoning(){
        ...
        let rejects = this.rejectionzones
        let diffs = []
...
/** second consolidation method, meant to strengthen consolidation identification */
    ...
    */
    ...
    static rejecinit(){
        ...
        let raw = fs.readFileSync('./Rejection_Archive/'+String(instrument)+'.json')
        this.timeperiods = JSON.parse(raw)
        this.rejectionzones = JSON.parse(raw)['Four_Hour']
    }
    /** Rejection Zone Saver */
    static rejecsave(){
        let instrument = Four_Hour_Functions.instrument_name()
        this.rejectionzones = [...new Set(this.rejectionzones)]
        this.timeperiods['Four_Hour'] = this.rejectionzones
        fs.writeFileSync('./Rejection_Archive/'+String(instrument)+'.json', JSON.stringify(this.timeperiods, null, 2))
    }
    ...
    static analysis(cases, extendedhistory, pricerange){
        ...
        if(this.rejectionzones.length < 1){
            this.rejectionzones.push(Four_Hour_Functions.price)
        }
        if(rejection > 2){
            return false
        }else{
