...
class Auto:
    ...
    def generate():
        ...
        for instrum in dict['instrument list']:
            Auto.inst = instrum
            threading.Thread(target=Auto.starter).start()

    def starter():
        instrument = Auto.inst
        dict = {}
        dict['instrument'] = instrument
        with open('instrument.json', 'w') as inst:
            json.dump(dict, inst)
        os.system('python3 -m venv ' + instrument +'\npython3 SparkPlug.py')
