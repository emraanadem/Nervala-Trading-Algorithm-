...
class History:
    ...
    def Equalizer(instrument):
        ...
        data = supabase.table('Five_Min Extend') \
            .insert({"id": ids, "Instrument": instrument, "Data": History.five, "OHLC": "c"}) \
            .execute()
        data = supabase.table('Fifteen_Min Extend') \
            .insert({"id": ids, "Instrument": instrument, "Data": History.fifteen, "OHLC": "c"}) \
            .execute()
        data = supabase.table('Thirty_Min Extend') \
            .insert({"id": ids, "Instrument": instrument, "Data": History.thirty, "OHLC": "c"}) \
            .execute()
        data = supabase.table('One_Hour Extend') \
            .insert({"id": ids, "Instrument": instrument, "Data": History.hour, "OHLC": "c"}) \
            .execute()
        data = supabase.table('Two_Hour Extend') \
            .insert({"id": ids, "Instrument": instrument, "Data": History.twohour, "OHLC": "c"}) \
            .execute()
        data = supabase.table('Four_Hour Extend') \
            .insert({"id": ids, "Instrument": instrument, "Data": History.fourhour, "OHLC": "c"}) \
            .execute()
        data = supabase.table('Daily Extend') \
            .insert({"id": ids, "Instrument": instrument, "Data": History.daily, "OHLC": "c"}) \
            .execute()
        data = supabase.table('Weekly Extend') \
            .insert({"id": ids, "Instrument": instrument, "Data": History.weekly, "OHLC": "c"}) \
            .execute()
