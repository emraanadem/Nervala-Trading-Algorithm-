...
class Daily_Nexus{
    ...
    /**pip logging method */
    static piplogger(){
        let piplogging = Daily_Nexus.piplog
    ...
    static takeProfitBuy(){
        if (Daily_Functions.price >= Daily_Nexus.tp){
            if(Daily_Functions.volatility() > .618){
    ...
    static stoplossdef(){
        let stoploss = Daily_Functions.stoploss()
        if(Daily_Nexus.buy_pos){
    ...
    static volatilitydef(){
        if(Daily_Functions.volatility() > .618 && Daily_Nexus.tstoplossinits && !Daily_Nexus.tstoplossvoid){
            Daily_Nexus.tstoplossdefvol()}
    }
    /** initiate trailing stop loss */
    static tstoplossinit(){
        let stoploss = Daily_Nexus.sldiff
        if(!Daily_Nexus.tstop && !Daily_Nexus.tstoplossinits && !Daily_Nexus.tstoplossvoid){
            if(Daily_Nexus.buy_pos){
                if(Daily_Functions.price > Daily_Nexus.posprice + .3*stoploss){
    ...
    static tstoplosscheck(){
        let tstoploss = Daily_Nexus.sldiff
        if(Daily_Nexus.buy_pos){
            if(Daily_Functions.price < Daily_Nexus.posprice + .3*tstoploss){
    ...
    static tstoplosscont(){
        if(Daily_Functions.volatility() < .618 && Daily_Nexus.tstoplossinits && !Daily_Nexus.tstoplossvoid){
            ...
            if(Daily_Nexus.buy_pos){
                if(Daily_Functions.price > Daily_Nexus.posprice + .3*stoploss){
                    Daily_Nexus.tstoploss = Daily_Nexus.posprice + Daily_Functions.pipreverse(Daily_Nexus.posprice, .618*Daily_Nexus.bigpipprice)
            }}
            if(Daily_Nexus.sell_pos){
                if(Daily_Functions.price < Daily_Nexus.posprice - .3*stoploss){
                    Daily_Nexus.tstoploss = Daily_Nexus.posprice - Daily_Functions.pipreverse(Daily_Nexus.posprice, .618*Daily_Nexus.bigpipprice)
    ...
    static buy(){
        ...
        if(!Daily_Functions.rejectionzoning()){
            if (Math.abs(Daily_Functions.valdiff(Daily_Functions.price, Daily_Functions.closest(Daily_Functions.price))) > .025) {
    ...
    static closePosTP(){
        if (Daily_Nexus.pos){
            if (Daily_Nexus.buy_pos){
                ...
                Daily_Nexus.piplog = [0, 0, 0]
                let pipchange = Daily_Functions.pipCountBuy(Daily_Nexus.posprice, Daily_Functions.price)
            ...
            if (Daily_Nexus.sell_pos){
                ...
                Daily_Nexus.trades += 1
                Daily_Nexus.piplog = [0, 0, 0]
                let pipchange = Daily_Functions.pipCountSell(Daily_Nexus.posprice, Daily_Functions.price)
                Daily_Nexus.pips += Math.abs(pipchange)
                console.log('pair: ' + Daily_Nexus.pair)
                console.log("Take Profit Hit on Daily")
                console.log(Daily_Nexus.wins + " Wins and     " + Daily_Nexus.losses + " Losses")
...
/** load historical prices from database */
    static async HistoryAssigner(){
        ...
        var { data, error} = await supabase
            ...
            .eq('Instrument', instrument)
            .eq('OHLC', 'c')
        Daily_Functions.extendHist = data[0]['Data']
        var { data, error} = await supabase
            .from('Daily Extend')
            .select('Data')
            .eq('Instrument', instrument)
            .eq('OHLC', 'h')
        Daily_Functions.extendHigh = data[0]['Data']
        var { data, error} = await supabase
            .from('Daily Extend')
            .select('Data')
            .eq('Instrument', instrument)
            .eq('OHLC', 'l')
        Daily_Functions.extendLow = data[0]['Data']}
        catch (error) {
            throw new Error(`Error: ${error.message}`);
            }
        console.log(Daily_Functions.extendHigh)
        let lens = []
        ...
        lens = []
        lens.push(Daily_Functions.extendHist.length)
        lens.push(Daily_Functions.extendHigh.length)
        lens.push(Daily_Functions.extendLow.length)
        minlens = Math.min(...lens)
        lists = [Daily_Functions.extendHist, Daily_Functions.extendHigh, Daily_Functions.extendLow]
        for (items in lists){
            if (items.length > minlens){
                if (items == Daily_Functions.extendHist){
                    for(let item = 0; item < (Daily_Functions.extendHist.length - minlens); item++){
                        Daily_Functions.extendHist.splice(0,1)
                    }
                if (items == Daily_Functions.extendLow){
                    for(let item = 0; item < (Daily_Functions.extendLow.length - minlens); item++){
                        Daily_Functions.extendLow.splice(0,1)
                    }
                if (items == Daily_Functions.extendHigh){
                    for(let item = 0; item < (Daily_Functions.extendHigh.length - minlens); item++){
                        Daily_Functions.extendHigh.splice(0,1)
                    }}}}}}
        }
...
/** second consolidation method, meant to strengthen consolidation identification */
    static consolidationtwo(){
        ...
        let highs = Daily_Functions.highs
        let lows = Daily_Functions.lows
 ...
 */
    static tpvariation(){
        let tp = Daily_Nexus.tp
        let values = Daily_Nexus.finlevs.concat(Daily_Nexus.biggersupres)
        let valdiffgreater = []
        let valdiffless = []
        let closesttp = 0
        let filteredvaldiff = []
        let nexttp = 0
        let referenceval = 0
        let num1 = Daily_Nexus.price
        let volval = Daily_Functions.volatility()
        if(Daily_Nexus.buy_pos){
    ...
    */
    static fib(){
        let recents = Daily_Functions.recentHisto
        let dataset = []
        for(let x = 1; x < 51; x++){
        ...
        let baseprice = 0
        let currentprice = Daily_Functions.price
    ...
    static overall(){
        let extendedhistory = Daily_Functions.extendHist
        ...
        let result = Daily_Functions.analysis(studylist, extendedhistory, pricerange)
        return result
    }
    ...
    static analysis(cases, extendedhistory, pricerange){
        let histnorm = Daily_Functions.priceHist
...
/** Smart array that grows as program runs longer for each time period, shows rejection zones and if the program is near them, it'll not allow trading */
    static rejectionzoning(){
        ...
        let rejects = Daily_Functions.rejectionzones
        let diffs = []
...
/** determination of stop loss size */
    static stoploss(){
        var highs = Daily_Functions.highs
        var lows = Daily_Functions.lows
        var diff = []
        var totaldiff = 0
        var finaldiff = 0
        ...
        let newsl = 0
        if(Daily_Nexus.pot_buy){
            let diffprice = Daily_Functions.price - finaldiff
            if(!Number.isFinite(Daily_Functions.closesttwo(diffprice)[0])){
                ...
                if(!Number.isFinite(Daily_Functions.closesttwo(numbuy)[0])){
                    ...
                    slfloor = (Daily_Functions.price-((Daily_Functions.price - Daily_Functions.closesttwo(numbuy)[0])*1.618*.786))
                    newsl = slfloor
                }}
            Daily_Nexus.sl = newsl
        }if(Daily_Nexus.pot_sell){
            let diffprice = finaldiff + Daily_Functions.price
            if(!Number.isFinite(Daily_Functions.closesttwo(diffprice)[1])){
                ...
                if(!Number.isFinite(Daily_Functions.closesttwo(numbuy)[1])){
                    ...
                    slceil = (Daily_Functions.price+((Math.abs(Daily_Functions.price - Daily_Functions.closesttwo(numbuy)[1]))*1.618*.786))
                    newsl = slceil
                }}
            Daily_Nexus.sl = newsl
            }
...
/** finds closest support and resistance level to whatever price u put in */
    static closesttwo(num1){
        ...
        for(let item = 0; item < values.length; item++){
            ...
            if (num1 > values[item]) {
                valdiffless.push(Math.abs(num1-values[item]))}}
        let closestbelow = Daily_Functions.price-Math.min(...valdiffless)
        let closestabove = Daily_Functions.price+Math.min(...valdiffgreater)
...
/** price zones, meant to determine whether a price zone has been found or not */
    static priceZones(){
        ...
        if(Math.abs((Daily_Functions.pipCountBuy(Daily_Functions.price,Daily_Nexus.resistance))
            )/(Math.abs(Daily_Functions.pipCountBuy(Math.max(...Daily_Functions.priceHist),Math.min(...Daily_Functions.priceHist)))) < .1){
            return true
...
/** keylev, meant to determine the closest keylevel to the current price */
    static keylev(){
        Daily_Functions.getPrice()
        if(Daily_Functions.valdiff(Daily_Functions.price, Daily_Functions.closest(Daily_Functions.price)) < .1){
            return true}
        else{
            return false}}
...
/** trailing stop loss factor, uses derived equation to create a sort of "bubble" around price movement to prevent trades being taken out early */
    static trailingsl(){
        var factor = Daily_Functions.volatility()
        let history = Daily_Functions.priceHist
        let ceiling = Math.max(...history)
        let floor = Math.min(...history)
        let diffy = ceiling - floor
        let posdiff = Math.abs(Daily_Nexus.posprice - Daily_Functions.price)
...
/**  used to determine price channels and send to announcer so that the announcer can announce whether a price channel has been found, similar to price zones */
    static priceChannels(){
        let rvalues = Daily_Functions.regression()
...
/** used to determine slope between two points */
    static slopes(){
        ...
        let recentHistory = Daily_Functions.recentHisto
        let slope = []
    ...
    static maxes_mins(){
        Daily_Functions.recentHist()
        let recentHistory = Daily_Functions.recentHisto
        let slope = Daily_Functions.slopes()
        let maxes = []
        let mins = []
...
/** finds support and resistance levels, very important for code function, would love to improve this */
    static supreslevs(){
        ...
        finalLevs = levelsss
        let price = Daily_Functions.getPrice()
        let larger = []
        let smaller = []
        let largertwo = []
        let smallertwo = []
...
/** pip converter */
    static pipreverse(num, num2){
        ...
        }else{Daily_Functions.multiplier = .0001}
        num2 *= Daily_Functions.multiplier
        return(num2)}
