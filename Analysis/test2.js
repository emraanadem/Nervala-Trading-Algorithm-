class Fifteen_Min_Functions {
  ...
  static instrument_name () {
    const raw = fs.readFileSync('instrument.json')
    const instrument = JSON.parse(raw)
    const dataspecific = instrument.instrument
    return dataspecific
  }
...
class Five_Min_Functions {
  ...
  /** load historical prices from json file */
  static async HistoryAssigner () {
    const instrument = Five_Min_Functions.instrument_name()
    try {
      var { data, error } = await supabase
        .from('Five_Min')
        .select('Data')
